...
__global__ void addVectorsGPU(float *a, float *b, float *c, int n)
{
	...
	int idx = threadIdx.x; 
  // store thread ID for each thread, so it knows where to start 
	int stride = blockDim.x;
  // the threads that complete all computations after 1024 will jump with the block size as their incremnet

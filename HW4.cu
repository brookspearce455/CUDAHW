...
void setUpDevices()
{
	BlockSize.x = 1024; //I incremented by a warp size of 32 and the max threads per block was the fastest with this given algorithm
	BlockSize.y = 1;
	BlockSize.z = 1;
...
__global__ void addVectorsGPU(float *a, float *b, float *c, int n)
{
	...
	int i;
	#pragma unroll  //This will unfold the for loop to reduce overhead and let us get a better comparison
	for(i = 0;i < N; i+= blockDim.x) // Making sure we are not working on memory we do not own.
	{
		if(id < N)
		{

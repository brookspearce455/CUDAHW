...
#define BS 256 // Set Block length
int BN = (N-1)/BS + 1; // Minimum blocks needed
...
{
	int id = threadIdx.x + blockIdx.x * blockDim.x; // Will calculate the Id for any given thread in one dimension
...
int main()
{
	...
	cudaErrorCheck(__FILE__,__LINE__); // check for errors after the vectors are added on the GPU
